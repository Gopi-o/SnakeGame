...
namespace SnakeGame
{
	void InitGameStateRecords(GameStateRecordsData& data, Game& game)
	{
		...
		data.tableTexts.reserve(MAX_RECORDS_TABLE_SIZE * 2);
		...
		for (int i = 0; i < MAX_RECORDS_TABLE_SIZE * 2 && it != sortedRecordsTable.rend(); ++i, ++it) // Note, we can do several actions in for action block
		{
		...
		data.hintText.setString("Press B to return back to main menu");
	...
	void DrawGameStateRecords(GameStateRecordsData& data, Game& game, sf::RenderWindow& window)
	{
		...
		sf::Vector2f tablePosition = { data.titleText.getGlobalBounds().left, viewSize.y / 2.5f };
