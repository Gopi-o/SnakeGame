...
namespace SnakeGame
{
	void InitGame(Game& game)
	{
		...
		assert(game.EscButtonSoundBuffer.loadFromFile(SOUNDS_PATH + "Esc.wav"));
		assert(game.TouchButtonSoundBuffer.loadFromFile(SOUNDS_PATH + "Touch.wav"));
		assert(game.GuidButtonSoundBuffer.loadFromFile(SOUNDS_PATH + "Guid.wav"));
		assert(game.GameBackground.openFromFile(MUSIC_PATH + "Game.wav"));
		assert(game.MenuBackground.openFromFile(MUSIC_PATH + "Menu.wav"));

		game.EscButtonSound.setBuffer(game.EscButtonSoundBuffer);
		game.TouchButtonSound.setBuffer(game.TouchButtonSoundBuffer);
		game.GuidButtonSound.setBuffer(game.GuidButtonSoundBuffer);
		game.GameBackground.setLoop(true);
	...
	{
		game.bisMusicOn = ((std::uint8_t)game.options & (std::uint8_t)GameOptions::Music) != (std::uint8_t)GameOptions::Empty;
		game.bIsSoundOn = ((std::uint8_t)game.options & (std::uint8_t)GameOptions::Sound) != (std::uint8_t)GameOptions::Empty;
		switch (state.type)
		{
		case GameStateType::MainMenu:
		{
